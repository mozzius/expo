...
<ConfigPluginProperties
  properties={[
    ...
    },
    {
      ...
      platform: 'ios',
      description:
        'A route to deep link to when the user taps the in-app notification settings link that iOS shows for your app in the system Settings app. Setting this enables `providesAppNotificationSettings`, so the link is displayed after the user is asked for notification permissions. The route is opened as a deep link using your app\'s URL scheme, so a `scheme` must be set in your app config.',
    },
  ]}
/>
